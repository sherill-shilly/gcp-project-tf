...
resource "google_container_cluster" "tools_cluster_use1" {
  name      = "gke-tools-cluster-use1"
  project   = var.project
  location  = "us-east1"
  deletion_protection = false
  ...
  network    = google_compute_network.gke_main_vpc.id
  subnetwork = google_compute_subnetwork.gke_app_use1.id
  ...
  ip_allocation_policy {
    cluster_secondary_range_name  = google_compute_subnetwork.gke_app_use1.secondary_ip_range[0].range_name
    services_secondary_range_name = google_compute_subnetwork.gke_app_use1.secondary_ip_range[1].range_name
  }

  node_config {
    tags = [local.tools_use1]
  }

}

resource "google_container_node_pool" "tools_nodepool_use1" {
  name     = "tools-nodepool-use1"
  location  = "us-east1"
  project   = var.project
  cluster  = google_container_cluster.tools_cluster_use1.name
  max_pods_per_node = 32
